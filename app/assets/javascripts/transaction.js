var app = angular.module("Hi5Exchange", ["ngResource", 'headings']);

app.factory('Recycler', ['$resource', function($resource){
  return $resource('/recyclables/:id.json', {id: '@id'}); //, {update: {method: 'PUT'}}
}]);

app.controller('RecyclerController', ['$scope', '$resource', 'Recycler',
  function($scope, $resource, Recycler){
    $scope.recyclables = Recycler.query();
  }
]);
